...
export default {
  ...
  setup(...props) {
        ...
        if (status === kakao.maps.services.Status.OK) {
          ...
          if (!doubleCheck) {
            if (document.getElementById("keyword")) {
              searchKeyWord.value = infoData.value.campingName;
              document.getElementById("keyword").value = searchKeyWord.value;
              searchPlaces();
    ...
    onMounted(() => {
      campingIdx.value = route.path.split("/")[2];
      initMap();
      getCampingData();
...
<template>
  <div class="map-box">
    <div class="map_wrap">
      <div id="map" style="width: 100%; border-radius: 15px; height: 100%; position: relative; overflow: hidden"></div>
    ...
    <!--    <button @click="ck">{{ infoData ? infoData.tel : "" }}</button>-->
  </div>
</template>
